% Copyright (C) 2019 David V谩zquez-Pad铆n, Marco Fontani, Dasara Shullani
%
...
global PATH_TO_EXPORT_MVS;   % 此处应该放编译后的exe文件？才可能解决找不到路径问题？-曹
...
if strcmp(strtrim(ext), '.mov') || strcmp(strtrim(ext), '.mp4') || strcmp(strtrim(ext), '.3gp') || strcmp(strtrim(ext), '.h264') || strcmp(strtrim(ext), '.mpeg')
    ...
    if numel(FRAME_types) == 0 && numel(mb_types) == 0
        ...
        eval(['!' PATH_TO_EXPORT_MVS ' ' tmp_video ' > ' mv_temp_file ' 2> /dev/null']); % PATH_TO_EXPORT_MVS处应该放编译后的exe文件？才可能解决找不到路径问题？-曹
